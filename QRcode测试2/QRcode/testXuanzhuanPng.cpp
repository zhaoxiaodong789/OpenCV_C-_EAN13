...
int main(int argc, char *argv[])
{
	...
	imshow("1.原图像", image);
	imwrite("./picture/1原图像.png", image);

	//2. 转化为灰度图  
	cvtColor(image, imageGray, CV_RGB2GRAY);
	imshow("2.灰度图", imageGray);
	imwrite("./picture/2灰度图.png", imageGray);

	//3. 高斯平滑滤波  
	GaussianBlur(imageGray, imageGuussian, Size(3, 3), 0);
	imshow("3.高斯平衡滤波", imageGuussian);
	imwrite("./picture/3高斯平滑滤波.png", imageGuussian);
	...
	imwrite("./picture/4X方向梯度.png", imageSobelX);
	imwrite("./picture/4Y方向梯度.png", imageSobelY);
	imwrite("./picture/4XY方向梯度差.png", imageSobelOut);


	//5.均值滤波，消除高频噪声  
	blur(imageSobelOut, imageSobelOut, Size(3, 3));
	imshow("5.均值滤波", imageSobelOut);
	imwrite("./picture/5均值滤波.png", imageSobelOut);

	//6.二值化  
	Mat imageSobleOutThreshold;
	threshold(imageSobelOut, imageSobleOutThreshold, 50, 255, CV_THRESH_BINARY);
	imshow("6.二值化", imageSobleOutThreshold);
	imwrite("./picture/6二值化.png", imageSobleOutThreshold);

	//7.闭运算，填充条形码间隙  
	Mat  element = getStructuringElement(0, Size(7, 7));
	morphologyEx(imageSobleOutThreshold, imageSobleOutThreshold, MORPH_CLOSE, element);
	imshow("7.闭运算", imageSobleOutThreshold);
	imwrite("./picture/7闭运算.png", imageSobleOutThreshold);
	...
	imwrite("./picture/8腐蚀.png", imageSobleOutThreshold);
	...
	imwrite("./picture/9膨胀.png", imageSobleOutThreshold);
	...
	imwrite("./picture/10找出二维码矩形区域.png", image);
	...
	imwrite("./picture/12旋转.png", xuanzhuanImg);
